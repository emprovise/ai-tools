...
public class JwnlWordNetApi {
	 ...
	 * using the default file_properties.xml file.
	 *
	 * NOTE: FOR WINDOWS INSTALL WINDOWS WORDNET 2.1 AND UPDATE THE "dictionary_path" IN file_properties.xml.
	 */
	public JwnlWordNetApi() {
		this(JwnlWordNetApi.class.getClassLoader().getResource("wordnet/file_properties.xml").getFile());
	}
	 ...
	 */
	public JwnlWordNetApi(String properties) {
		try {
			File file = new File(properties);
			JWNL.initialize(new FileInputStream(file));
			dict = net.didion.jwnl.dictionary.Dictionary.getInstance();
			System.out.println(dict.lookupAllIndexWords("test").getIndexWordArray());
		} catch (Exception e) {
	...
	/**
	 * Checks if the word exists as the specified part of speech.
	 *
	 * @param word a word
	 * @return <code>true</code> iff the word matches the specified part of speech
	 */
	public boolean isPartOfSpeech(POS pos, String word) {
		if (dict == null) return false;
	...
	public boolean isAdjective(String word) {
		return isPartOfSpeech(POS.ADJECTIVE, word);
	}
	...
	public boolean isAdverb(String word) {
		return isPartOfSpeech(POS.ADVERB, word);
	}
	...
	public boolean isNoun(String word) {
		return isPartOfSpeech(POS.NOUN, word);
	}
	...
	public boolean isVerb(String word) {
		return isPartOfSpeech(POS.VERB, word);
	}
...
//		return isCompoundWord(word);
	...
	public static void main(String[] args) {
		JwnlWordNetApi jwnlWordNetApi = new JwnlWordNetApi();
		System.out.println(jwnlWordNetApi.expandEntity("fire"));
	}
}
